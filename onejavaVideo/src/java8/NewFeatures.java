...
public class NewFeatures {
	...
	public static void main(String[] args) {
		...
		users.stream().forEach(User::talk); //ikisi de ayný þeyi ifade eder
		
		List<User> filteredList = users.stream().filter(user -> user.id>5)
				.collect(Collectors.toList()); //id si 5 ten büyük olanlarý filter edip yeni list içine atamak
		System.out.println("Filtered");
		filteredList.stream().forEach(User::talk);
		
		List<User> mappedList =  users.stream()
			.map(user -> new User(user.id+100,user.name)) //elimizdeki userdan yeni bir user oluþturup id sini 100 artýrdýk
